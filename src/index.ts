let displayAddNewTask = document.getElementById(
  "displayAddNewTask",
...
let closeInputSectionBtn = document.getElementById(
  "closeInputSectionBtn",
) as HTMLButtonElement;
let displaysetAlarm = document.getElementById(
  "displaysetAlarm",
) as HTMLButtonElement;
let setAlarmTime = document.getElementById("setAlarmTime") as HTMLInputElement;
let alarmTimeDisplay = document.getElementById(
  "alarmTimeDisplay",
) as HTMLParagraphElement;
let closealarmSectionBtn = document.getElementById(
  "closealarmSectionBtn",
) as HTMLButtonElement;
let alarmPlaceHolder = document.getElementById(
  "alarmPlaceHolder",
) as HTMLParagraphElement;
let alarmPlaceHolderSection = document.getElementById(
  "alarmPlaceHolderSection",
) as HTMLDivElement;
let addTaskBtn = document.getElementById("addTaskBtn") as HTMLButtonElement;
let removeSettedAlarmBtn = document.getElementById(
  "removeSettedAlarmBtn",
) as HTMLButtonElement;
let incompleteTaskList = document.getElementById(
  "incompleteTaskList",
) as HTMLUListElement;
let completedTaskList = document.getElementById(
  "completedTaskList",
) as HTMLUListElement;
...
let searchInputSection = document.getElementById(
  "searchSection",
) as HTMLDivElement;
let searchInput = document.getElementById("searchInput") as HTMLInputElement;
let listDiscription = document.getElementById(
  "listDiscription",
) as HTMLSpanElement;
let closeEditSectionBtn = document.getElementById(
  "closeEditSectionBtn",
) as HTMLButtonElement;
let editTaskInput = document.getElementById(
  "editTaskInput",
) as HTMLInputElement;
let editAlarmInput = document.getElementById(
  "editAlarmInput",
) as HTMLInputElement;
let saveEditedTaskBtn = document.getElementById(
  "saveEditedTaskBtn",
) as HTMLButtonElement;
...
const SaveTaskFun = () => {
  ...
  let countId: number =
    toDoLists.length > 0 ? (toDoLists[toDoLists.length - 1]?.id ?? 0) + 1 : 1;
...
const renderTasks = (searchTerm: string = "") => {
  ...
  const createLi = (task: Task) => {
    ...
    const textStyle = task.completed
      ...
      : "";
    const displayTime = task.alarmTime
      ? task.alarmTime.replace("T", " ")
      : "No alarm";
  ...
  pendingTasks.forEach((task) =>
    incompleteTaskList.appendChild(createLi(task)),
  );
